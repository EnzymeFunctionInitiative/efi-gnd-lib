...
use Efi\Gnd\Logic\LayoutMapper;
use Efi\Gnd\Logic\SequenceAttributeParser;
use Efi\Gnd\Interface\GndReaderInterface;
use \PDO;
use \PDOException;

class GndReader implements GndReaderInterface
{
    ...
    public function retrieveRanges(string $rangeParameter, GndQueryParams $params): array
    {
        ...
        foreach ($this->iterateRawGndData($indices, $params) as $row) {
            $coordHelper->updateCoords($row['attributes']);
            foreach ($row['neighbors'] as $nb) {
    ...
    /**
     * Provide iterator interface to database retrieval.
     *
     ...
     *    }
     */
    public function iterateRawGndData(array $indices, GndQueryParams $params): \Generator
    {
        $attrParser = new SequenceAttributeParser();
        ...
        foreach ($indices as $index) {
            ...
            yield ['attributes' => $queryData, 'neighbors' => $neighborData];
        }
    }
    ...
    private function getLayoutMapper(GndQueryParams $params): LayoutMapper
    {
        if ($this->layoutMapper !== null) {
            return $this->layoutMapper;
        }
        $this->layoutMapper = new LayoutMapper($this->getPdo(), $params->scaleFactor);
        return $this->layoutMapper;
    }
    ...
    private function getSearchIndexRanges(array $searchItems, GndQueryParams $params): array
    {
        ...
        $sequenceIdLookup = new \Efi\Gnd\Logic\IdLookup\SequenceId($this->getPdo(), $params->sequenceVersion);
